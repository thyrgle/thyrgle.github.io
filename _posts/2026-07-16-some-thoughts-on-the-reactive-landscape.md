...
This does not remind me of either the Wikipedia article or spreadsheets. There are no "events". What is the purpose of `subscribe`? Frankly, none of the examples in their [overview](https://rxjs.dev/guide/overview) strike me as resembling *the essence* of reactive programming. That is *not* to say there is no use case for RxJS, it just did not strike me as "reactive programming" at first glance. A closer look does reveal you can create values that update based on other values, but the syntax is filled with lambda functions and lacks the simplicity of, `a = b + c`. I *want that* simplicity.

Before continuing, let's also look at a [counter example](https://www.solidjs.com/examples/counter) that uses another reactive library [Solid.js:](https://www.solidjs.com/)
...
This counter example increments a value every second. It still strikes me as too verbose. Here is what I envision it should look like (although, it does not run!)
...
- There is no need for `setCount`.
- `count` can be used like a normal variable. You can increment it, and do any arithmetic you want on it.

This might sound small, but, in my experience, even small excess syntax can build up and hinder readability. I think we really need to consider (especially since AI can churn out code so quickly) getting rid of awkward syntax *as much as possible*. My idea?

> What if things were more like a spreadsheet?

# Solving the Problem

I started working on [pypagate](https://github.com/thyrgle/pypagate) trying to understand what I envisioned reactive programming to look like. Like I mentioned above, the core tenet is that *assignment becomes mathematical equality*. So, once `z = x + y`, `z` can change value depending on what value `x` and `y` are. Thus, I came up with the following scheme:
...
(Once again, just to emphaise, this code *does not* work, it's just what I envision!)
...
Unfortunately, the world beyond my libraries are *not* reactive. So UI elements *do not* change when my formula/terms change.

Hence, my future work will be making UI frameworks more reactive friendly to allow for this kind of programming. Can a UI item change it's text? Then the text should be representable by a term or formula that changes to reflect the underlying term or formula changes as well. No boilerplate, just change. 
